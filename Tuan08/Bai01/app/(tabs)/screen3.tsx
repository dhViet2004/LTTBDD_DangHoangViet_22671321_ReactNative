...
export default function AddJobScreen() {
  ...
  const handleFinish = async () => {
    ...
    try {
      ...
      if (id) {
        // ✅ Update task theo id
        await db.runAsync('UPDATE tasks SET title = ? WHERE id = ?', [job, id]);
      } else {
        // ✅ Thêm task mới
        await addTask(job);
      }
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: '#FAFAFC' }}>
      ...
      {/* Button */}
      <TouchableOpacity
        style={[styles.button, { opacity: loading || !job.trim() ? 0.6 : 1 }]}
      ...
      {/* Note Image */}
      <Image source={{ uri: noteImage }} style={styles.noteImage} resizeMode="contain" />
    </SafeAreaView>
  );
}
